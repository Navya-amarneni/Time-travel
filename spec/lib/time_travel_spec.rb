  ...
  context 'multiple identifiers and batch size not defined' do
    ...
    describe 'create' do
      it "creates new record with timestamps set" do
        table = entry_klass.create(amount: amount, req_id: req_id, req_type: req_type, wrapper_id: wrapper_id,
          effective_from: effective_from, effective_till: TimeTravel::INFINITE_DATE)
    ...
    describe 'delete' do
      ...
      it "makes current effective record ineffective" do
        entry = entry_klass.create(amount: amount, req_id: req_id, req_type: req_type, wrapper_id: wrapper_id,
          effective_from: effective_from, effective_till: TimeTravel::INFINITE_DATE)
        entry.delete(effective_till: Time.current)
        entry_history = entry_klass.as_of(Time.current, wrapper_id, req_id, req_type)
        expect(entry_history).to eql(nil)
        entry_history = entry_klass.as_of(Time.current - 1.hour, wrapper_id, req_id, req_type)
      ...
      it "reports error when there is no effective record found" do
        ...
        expect(table.reload.delete(effective_till: deleted_time)).to be_falsy
      end
    end
  end
